...
if __name__ == "__main__":
    ...
    parser.add_argument("--port", type=int, default=50025, help="接続を受け付けるポート番号です。")
    parser.add_argument(
        "--use_gpu", action="store_true", help="指定するとGPUを使って音声合成するようになります。"
    )
    parser.add_argument(
        "--sharevox_dir", type=Path, default=None, help="SHAREVOXのディレクトリパスです。"
    )
    parser.add_argument(
        "--voicelib_dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=(
            "音声合成を行うスレッド数です。指定しないと、代わりに環境変数SV_CPU_NUM_THREADSの値が使われます。"
            "SV_CPU_NUM_THREADSが空文字列でなく数値でもない場合はエラー終了します。"
        ),
    )
