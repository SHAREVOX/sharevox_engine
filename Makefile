...
build-linux-docker-ubuntu20.04:
	docker buildx build . \
		-t sharevox/sharevox_engine:cpu-ubuntu20.04-latest \
		--target runtime-env \
		--progress plain \
...
run-linux-docker-ubuntu20.04:
	docker run --rm -it \
		...
		sharevox/sharevox_engine:cpu-ubuntu20.04-latest $(CMD)

.PHONY: build-linux-docker-nvidia-ubuntu20.04
build-linux-docker-nvidia-ubuntu20.04:
	docker buildx build . \
		-t sharevox/sharevox_engine:nvidia-ubuntu20.04-latest \
...
run-linux-docker-nvidia-ubuntu20.04:
	docker run --rm -it \
		--gpus all \
		-p '127.0.0.1:50021:50021' $(ARGS) \
		sharevox/sharevox_engine:nvidia-ubuntu20.04-latest $(CMD)
...
build-linux-docker-ubuntu18.04:
	docker buildx build . \
		-t sharevox/sharevox_engine:cpu-ubuntu18.04-latest \
		--target runtime-env \
		--progress plain \
...
run-linux-docker-ubuntu18.04:
	docker run --rm -it \
		-p '127.0.0.1:50021:50021' $(ARGS) \
		sharevox/sharevox_engine:cpu-ubuntu18.04-latest $(CMD)

.PHONY: build-linux-docker-nvidia-ubuntu18.04
build-linux-docker-nvidia-ubuntu18.04:
	docker buildx build . \
		-t sharevox/sharevox_engine:nvidia-ubuntu18.04-latest \
		--target runtime-nvidia-env \
		--progress plain \
		--build-arg BASE_IMAGE=ubuntu:bionic \
...
run-linux-docker-nvidia-ubuntu18.04:
	docker run --rm -it \
		--gpus all \
		-p '127.0.0.1:50021:50021' $(ARGS) \
		sharevox/sharevox_engine:nvidia-ubuntu18.04-latest $(CMD)
...
build-linux-docker-download-core-env-ubuntu18.04:
	docker buildx build . \
		-t sharevox/sharevox_engine:download-core-env-ubuntu18.04 \
...
run-linux-docker-download-core-env-ubuntu18.04:
	docker run --rm -it $(ARGS) \
		sharevox/sharevox_engine:download-core-env-ubuntu18.04 $(CMD)
...
build-linux-docker-download-onnxruntime-env-ubuntu18.04:
	docker buildx build . \
		-t sharevox/sharevox_engine:download-onnxruntime-env-ubuntu18.04 \
...
run-linux-docker-download-onnxruntime-env-ubuntu18.04:
	docker run --rm -it $(ARGS) \
		sharevox/sharevox_engine:download-onnxruntime-env-ubuntu18.04 $(CMD)
...
build-linux-docker-compile-python-env:
	docker buildx build . \
		-t sharevox/sharevox_engine:compile-python-env \
...
run-linux-docker-compile-python-env:
	docker run --rm -it $(ARGS) \
		sharevox/sharevox_engine:compile-python-env $(CMD)
...
build-linux-docker-build-ubuntu20.04:
	docker buildx build . \
		-t sharevox/sharevox_engine:build-cpu-ubuntu20.04-latest \
		--target build-env \
		--progress plain \
		--build-arg BASE_IMAGE=ubuntu:focal \
		--build-arg BASE_RUNTIME_IMAGE=ubuntu:focal \
...
run-linux-docker-build-ubuntu20.04:
	docker run --rm -it \
		-v "$(shell pwd)/build:/opt/sharevox_engine_build" $(ARGS) \
		sharevox/sharevox_engine:build-cpu-ubuntu20.04-latest $(CMD)

.PHONY: build-linux-docker-build-nvidia-ubuntu20.04
build-linux-docker-build-nvidia-ubuntu20.04:
	docker buildx build . \
		-t sharevox/sharevox_engine:build-nvidia-ubuntu20.04-latest \
		--target build-env \
		--progress plain \
		--build-arg BASE_IMAGE=ubuntu:focal \
		--build-arg BASE_RUNTIME_IMAGE=nvidia/cuda:11.4.2-cudnn8-runtime-ubuntu20.04 \
...
run-linux-docker-build-nvidia-ubuntu20.04:
	docker run --rm -it \
		-v "$(shell pwd)/build:/opt/sharevox_engine_build" $(ARGS) \
		sharevox/sharevox_engine:build-nvidia-ubuntu20.04-latest $(CMD)

## Ubuntu 18.04
.PHONY: build-linux-docker-build-ubuntu18.04
build-linux-docker-build-ubuntu18.04:
	docker buildx build . \
		-t sharevox/sharevox_engine:build-cpu-ubuntu18.04-latest \
		--target build-env \
		--progress plain \
		--build-arg BASE_IMAGE=ubuntu:bionic \
		--build-arg BASE_RUNTIME_IMAGE=ubuntu:bionic \
...
run-linux-docker-build-ubuntu18.04:
	docker run --rm -it \
		-v "$(shell pwd)/build:/opt/sharevox_engine_build" $(ARGS) \
		sharevox/_engine:build-cpu-ubuntu18.04-latest $(CMD)

.PHONY: build-linux-docker-build-nvidia-ubuntu18.04
build-linux-docker-build-nvidia-ubuntu18.04:
	docker buildx build . \
		-t sharevox/sharevox_engine:build-nvidia-ubuntu18.04-latest \
		--target build-env \
		--progress plain \
		--build-arg BASE_IMAGE=ubuntu:bionic \
		--build-arg BASE_RUNTIME_IMAGE=nvidia/cuda:11.4.2-cudnn8-runtime-ubuntu18.04 \
...
run-linux-docker-build-nvidia-ubuntu18.04:
	docker run --rm -it \
		-v "$(shell pwd)/build:/opt/sharevox_engine_build" $(ARGS) \
		sharevox/sharevox_engine:build-nvidia-ubuntu18.04-latest $(CMD)
