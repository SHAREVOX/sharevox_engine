...
[こちら](https://github.com/SHAREVOX/sharevox_engine/releases/latest)から対応するエンジンをダウンロードしてください。
...
curl -s \
    -X POST \
    "127.0.0.1:50025/audio_query?speaker=1"\
...
curl -s \
    ...
    -X POST \
    -d @query.json \
    "127.0.0.1:50025/synthesis?speaker=1" \
    > audio.wav
```
...
curl -s \
    -X POST \
    "127.0.0.1:50025/accent_phrases?speaker=1&is_kana=true" \
...
```bash
curl -s -X GET "127.0.0.1:50025/user_dict"
```
...
curl -s -X POST "127.0.0.1:50025/user_dict_word" \
    --get \
...
curl -s -X PUT "127.0.0.1:50025/user_dict_word/$word_uuid" \
    --get \
    --data-urlencode "surface=$surface" \
    --data-urlencode "pronunciation=$pronunciation" \
    --data-urlencode "accent_type=$accent_type"
...
curl -s -X DELETE "127.0.0.1:50025/user_dict_word/$word_uuid"
```
...
# プリセット情報を取得
curl -s -X GET "127.0.0.1:50025/presets" > presets.json
...
curl -s \
    -X POST \
    "127.0.0.1:50025/audio_query_from_preset?preset_id=$preset_id"\
    --get --data-urlencode text@text.txt \
    > query.json
...
curl -s \
    ...
    -X POST \
    -d @query.json \
    "127.0.0.1:50025/synthesis?speaker=$style_id" \
    > audio.wav
```
...
curl -s \
    -X POST \
    "127.0.0.1:50025/audio_query?speaker=0"\
    --get --data-urlencode text@text.txt \
    > query.json
...
curl -s \
    ...
    -X POST \
    -d @query.json \
    "127.0.0.1:50025/synthesis?speaker=0" \
    > audio.wav
...
curl -s \
    ...
    -X POST \
    -d @query.json \
    "127.0.0.1:50025/synthesis_morphing?base_speaker=0&target_speaker=1&morph_rate=$MORPH_RATE" \
    > audio.wav
...
```bash
curl -s -X GET "127.0.0.1:50025/speaker_info?speaker_uuid=7ffcb7ce-00ec-4bdc-82cd-45a8889e43ff" \
    | jq  -r ".portrait" \
    | base64 -d \
    > portrait.png
```
